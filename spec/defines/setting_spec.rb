...
describe 'apt::setting' do
  ...
  describe 'when using the defaults' do
    ...
    context 'with title=conf-teddybear ' do
      ...
      it { is_expected.to contain_file('/etc/apt/apt.conf.d/50teddybear').that_notifies('Exec[aptget_update]') }
    end

    context 'with title=pref-teddybear' do
      let(:title) { 'pref-teddybear' }
      let(:params) { default_params }
      it { is_expected.to contain_file('/etc/apt/preferences.d/teddybear.pref').that_notifies('Exec[aptget_update]') }
    end

    context 'with title=list-teddybear' do
      let(:title) { 'list-teddybear' }
      let(:params) { default_params }
      it { is_expected.to contain_file('/etc/apt/sources.list.d/teddybear.list').that_notifies('Exec[aptget_update]') }
    end

    context 'with source' do
      let(:params) { { :source => 'puppet:///la/die/dah' } }
      it {
        is_expected.to contain_file('/etc/apt/apt.conf.d/50teddybear').that_notifies('Exec[aptget_update]').with({
        :ensure => 'file',
        :owner  => 'root',
        :group  => 'root',
        :mode   => '0644',
    ...
    context 'with content' do
      let(:params) { default_params }
      it { is_expected.to contain_file('/etc/apt/apt.conf.d/50teddybear').that_notifies('Exec[aptget_update]').with({
        :ensure  => 'file',
        :owner   => 'root',
        :group   => 'root',
        :mode    => '0644',
  ...
  describe 'with priority=100' do
    ...
    it { is_expected.to contain_file('/etc/apt/apt.conf.d/100teddybear').that_notifies('Exec[aptget_update]') }
  end
