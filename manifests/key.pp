# == Define: apt::key
define apt::key (
  # Example42 backward compatibility
  $url         = undef,
  $environment = undef,
  $path        = undef,
  $keyserver   = undef,
  $fingerprint = undef,
  # End Example42 backward compatibility
  $id          = $name,
  $ensure      = present,
  $content     = undef,
  $source      = undef,
  ...
  # Example42 backward compatibility
  if $url != undef and $url != '' {
    deprecation('apt $url', '$url is deprecated and will be removed in the next major release. Please use $source instead.')
    $_source42 = $url
  } else {
    $_source42 = $source
  }
  if $environment != undef {
    deprecation('apt $environment', '$environment is ignored.')
  }
  if $path != undef {
    deprecation('apt $path', '$path is ignored.')
  }
  if $keyserver != undef and $keyserver != '' {
    deprecation('apt $keyserver', '$keyserver is deprecated and will be removed in the next major release. Please use $server instead.')
    $_server42 = $keyserver
  } else {
    $_server42 = $server
  }
  if $fingerprint != undef and $fingerprint != '' {
    deprecation('apt $fingerprint', '$fingerprint is deprecated and will be removed in the next major release. Please use $id instead.')
    $_id42 = $fingerprint
  } else {
    $_id42 = $id
  }
